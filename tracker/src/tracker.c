...
static int run_mag_calibration(mag_calib_t *calib) {
    ...
    for (int pass = 0; pass < 3; pass++) {
        struct sensor_angle pan_cmd = {.angle = 0};
        orb_publish_multi(pan_fd, &pan_cmd, sizeof(pan_cmd));
        usleep(1000 * 3000);

        for (float angle = CONFIG_INSPACE_TRACKER_MIN_ANGLE; angle <= CONFIG_INSPACE_TRACKER_MAX_ANGLE; angle += 1.0f) {
            struct sensor_angle pan_cmd = {.angle = angle};
            orb_publish_multi(pan_fd, &pan_cmd, sizeof(pan_cmd));
...
int main(void) {
    ...
    mag_calib_t calib = {.hard_iron_x = 0.0f, .hard_iron_y = 0.0f, .soft_radius_x = 1.0f, .soft_radius_y = 1.0f};
