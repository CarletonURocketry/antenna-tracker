...
parsed_packet parse_packet(char *buffer) {
    ...
    */
    ...
    // Reasoning: If we don't have the position at a given time, the tracker might go into a bad state/return to default
    // position instead of staying where it is. memset(&packet, 0, sizeof(packet));
